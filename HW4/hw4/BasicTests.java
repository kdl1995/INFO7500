...
public class BasicTests extends TestCase {
	...
	public List<NodeFactory> BASIC_MALICIOUS_NODE_FACTORIES = Arrays.asList(MaliciousNode0.FACTORY,
			MaliciousNode1.FACTORY, MaliciousNode3.FACTORY);

	public List<String> BASIC_GRAPH_FILES = Arrays.asList("10_10_1", "10_20_1", "10_30_1", "10_40_1", "20_10_1",
			"20_20_1", "20_30_1", "30_10_1", "30_20_1", "30_30_1", "30_40_1");

	public List<String> BASIC_TX_FILES = Arrays.asList("1percent_1", "5percent_1", "10percent_1");

	public void test0() throws Exception {
		PrintWriter wr = new PrintWriter(new FileWriter(
				"test0_" + DateTimeFormatter.ofPattern("yyyy-MM-dd_HH-mm").format(LocalDateTime.now()) + ".csv"));
			...
			for (NodeFactory mnf : BASIC_MALICIOUS_NODE_FACTORIES) {
				for (String graphFile : BASIC_GRAPH_FILES) {
					for (String txFile : BASIC_TX_FILES) {
						Scenario scenario = ScenarioParser.parse(cnf, mnf, "files/" + graphFile + ".txt",
								"files/" + txFile + ".txt", 10);
	...
	public void writeStatistics(PrintWriter wr, List<ScenarioRunner> runners) {
		...
		wr.println("# Scenarios: Total=" + runners.size() + " AllNodesInConsensus=" + numScenarios_AllNodesInConsensus
				+ " (" + (numScenarios_AllNodesInConsensus * 100 / runners.size()) + "%) " + " AllTxsInConsensus="
				+ numScenarios_AllTxsInConsensus + " (" + (numScenarios_AllTxsInConsensus * 100 / runners.size())
				+ "%)");
